...
function __autoload($class) {
	$class = strtolower($class);//全部换成小写
	if (file_exists(WEB_ROOT . '/class/' . $class . '.php')) {
		require_once(WEB_ROOT . '/class/' . $class . '.php');
	}  else
		emMsg($class . '加载失败。');
...
/**
 * 去除多余的转义字符，stripslashesDeep删除反斜杠
 */
...
function stripslashesDeep($value) {
	...
	return $value;//array_map函数将用户自定义函数作用到数组中的每个值上，并返回用户自定义函数作用后的带有新值的数组。
}
...
function htmlClean($content, $nl2br = true) {
	$content = htmlspecialchars($content, ENT_QUOTES, 'UTF-8');//htmlspecialchars() 函数把预定义的字符转换为 HTML 实体。
	if ($nl2br) {
...
/**
 * 获取站点地址(仅限根目录脚本使用,目前仅用于首页ajax请求)preg_match正则表达式匹配
 */
function getBlogUrl() {
...
function emLoadJQuery() {
	...
	if (!$isJQueryLoaded) {
		...
		array_unshift($emHooks['index_head'], 'loadJQuery');//array_unshift() 函数用于向数组插入新元素。新数组的值将被插入到数组的开头。
		$isJQueryLoaded = true;

		function loadJQuery() {
			echo '<script src="' . WEB_URL . 'include/lib/js/jquery/jquery-1.7.1.js" type="text/javascript"></script>';
		}
...
function extractHtmlData($data, $len) {
	$data = strip_tags(subString($data, 0, $len + 30));//strip_tags() 函数剥去字符串中的 HTML、XML 以及 PHP 的标签。
	$search = array("/([\r\n])[\s]+/", // 去掉空白字符
...
function emoFormat($t){
	...
	if(!empty($t) && preg_match_all('/\[.+?\]/',$t,$matches)){
		...
		foreach ($matches as $data) {
			if(isset($emos[$data]))
				$t = str_replace($data,'<img title="'.$data.'" src="'.WEB_URL.'admin/editor/plugins/emoticons/images/'.$emos[$data].'"/>',$t);
		}
	}
	return $t;
}
